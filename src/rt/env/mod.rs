...
fn setup_number<'a>(env: &mut JsEnv, mut global: Local<JsValue>, function_prototype: Local<JsObject>) {
	...
	function!(&mut prototype, name::TO_LOCALE_STRING, Number_toLocaleString, 0, function_prototype, env);
	function!(&mut prototype, name::TO_FIXED, Number_toFixed, 1, function_prototype, env);
	function!(&mut prototype, name::TO_EXPONENTIAL, Number_toExponential, 1, function_prototype, env);
	function!(&mut prototype, name::TO_PRECISION, Number_toPrecision, 1, function_prototype, env);
}
